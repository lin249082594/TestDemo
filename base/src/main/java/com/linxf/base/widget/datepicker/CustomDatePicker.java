...
//    }
    ...
    public boolean setSelectedTime(String dateStr, boolean showAnim) {
        return canShow() && !TextUtils.isEmpty(dateStr)
                && setSelectedTime(DateFormatUtils.str2Long(dateStr, formatParrten), showAnim);
    }
    ...
    /**
     * type  1年月 2年月日 3年月日时分秒
     */
    public void setCanShowPreciseTime(String formatParrten) {
        if (!canShow()) return;
        this.formatParrten = formatParrten;
        if(DateFormatUtils.DATE_FORMAT_PATTERN_YM.equals(formatParrten)){
        ...
        } else if (DateFormatUtils.DATE_FORMAT_PATTERN_YMD.equals(formatParrten)) {
            initScrollUnit(SCROLL_UNIT_HOUR, SCROLL_UNIT_MINUTE);
            mDpvHour.setVisibility(View.GONE);
            mTvHourUnit.setVisibility(View.GONE);
            mDpvMinute.setVisibility(View.GONE);
            mTvMinuteUnit.setVisibility(View.GONE);
        } else if(DateFormatUtils.DATE_FORMAT_PATTERN_YMD_HM.equals(formatParrten)){
            initScrollUnit();
